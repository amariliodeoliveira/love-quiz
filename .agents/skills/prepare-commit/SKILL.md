...
1. Read `.claude/git-guidelines.md` before proposing or performing Git actions.
...
7. A request to implement, finish, or deliver authorizes focused commits and a direct push to `main` under `.claude/git-guidelines.md`. A request only to draft or plan does not mutate Git.
8. After pushing, monitor CI through completion. If `main` fails, fix forward when clear and safe; otherwise revert. Audit the remote commit and remaining working tree after success.
