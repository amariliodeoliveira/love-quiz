...
| `eslint-plugin-security`                   | Common Node/JS vulnerability patterns                             | 🟡 Keep, audit computed access | `detect-object-injection` is syntactic and broad; known-safe access is suppressed locally, while new access fails lint and requires review                                                                  |
...
- **2026-08-16** — Audited all 7 `security/detect-object-injection` warnings. None allowed
  user-controlled keys: they were bounded array access, closed union/tuple lookup, or keys from a fixed
  validator table. Promoted the rule to `error` and added named, local suppressions so any new computed
  access requires review instead of becoming permanent CI noise. The audit did uncover two adjacent
  correctness issues: injected random callbacks could violate their return contract, and the profile
  route accepted non-object JSON bodies until property access failed. Both now have regression tests.
- **2026-08-16** — Restricted pre-commit ESLint autofix to `layout` rules. Broad `eslint --fix` had
  already caused two Unicorn regressions; Prettier and import sorting remain automatic, while semantic
  lint findings must be fixed deliberately and reviewed. Added selected type-aware async rules
  (`no-floating-promises`, `no-misused-promises`, `await-thenable`) and disabled SonarJS's duplicate
  unused-import rule in favor of `eslint-plugin-unused-imports` as the single owner.
